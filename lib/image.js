...
exports.generateOutput = function (layerInfo) {
  ...
  layerInfo.items.forEach( function (item) {
    var sizedW = item.meta.content.width / (item.width - item.meta.gutter);
    var sizedH = item.meta.content.height / (item.height - item.meta.gutter);
    if (sizedW == sizedH) {
      ...
      var dx = item.x + item.meta.gutter;
      var dy = item.y + item.meta.gutter;
      var dwidth = item.width - 2 * item.meta.gutter;
      var dheight = item.height - 2 * item.meta.gutter;
      context.drawImage(item.meta.content, sx, sy, swidth, sheight, dx, dy, dwidth, dheight);
    }
  });
